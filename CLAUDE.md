...
- **`test/spec_coverage_test.dart`** runs the *reverse* direction: it scans the parser's dispatch tables (assembler `case`/`name ==` literals, the metadata name sets, formatting targets, chord qualities) and fails if any name is absent from both `chordpro-spec-checklist.md` and `doc/reference/non-spec-extensions.md`. Spec-outward audits are blind to non-spec extras — this is what catches them. Adding a directive/alias/quality means adding it to a ledger in the same change; the escape hatch is the (deliberately empty) `_allowlist` in that file.
